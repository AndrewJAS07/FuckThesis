...
const BookingScreen: React.FC = () => {
    ...
    if (params.bookingData) {
      try {
        ...
        setRegion({
          latitude: centerLat,
          longitude: centerLon,
          latitudeDelta: Math.abs(data.pickup.latitude - data.destination.latitude) * 1.5 || 0.01,
          longitudeDelta: Math.abs(data.pickup.longitude - data.destination.longitude) * 1.5 || 0.01,
        });

        createRide(data);
      } catch (e) {
        console.error('Invalid bookingData:', e);
        setError('Invalid booking information received.');
      }
    }
  }, [params.bookingData]);
  ...
  const handleCancelBooking = async () => {
    if (!rideId) {
      Alert.alert('Error', 'No ride ID found. Cannot cancel booking.');
      return;
    }
    try {
      setIsLoading(true);
